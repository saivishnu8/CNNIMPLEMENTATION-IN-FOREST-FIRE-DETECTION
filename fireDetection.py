...
def play_alarm_sound_function(): # defined function to play alarm post fire detection using threading
    playsound.playsound('fire_alarm.mp3',True) # to play alarm # mp3 audio file 
...
def send_mail_function(): # defined function to send mail post fire detection using threading
    ...
    recipientmail = "sunnyreddyanireddy1234@gmail.com" # recipients mail
    ...
    try:
        ...
        server.ehlo()
        server.starttls()
        server.login("vtu20012@veltech.edu.in", 'Sravan@0912 ') # Senders mail ID and password
        server.sendmail('bharadwajvemireddy@gmail.com', recipientmail, "Warning fire accident has been reported") # recipients mail with mail message
